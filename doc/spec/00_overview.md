...
エラー発生時はアラートで表示（`SportsNoteError` + `ErrorAlertModifier`）。エラー種別:
- Realmエラー（初期化、読み書き、削除、マイグレーション）
- Firebaseエラー（接続、認証、権限、ネットワーク等）
- ネットワークエラー（接続不可、タイムアウト）
- 予期しないエラー（システムエラー、原因不明のエラー、重大エラー等）

アラートの挙動はエラー種別に応じて変化する:
- タイトルのアイコン: 重大エラー（🚨）、システム/DBエラー（⚠️）、認証エラー（🔐）、ネットワークエラー（🌐）、その他（ℹ️）
- ボタン: 重大エラー・システムエラーは「OK」のみ、それ以外はキャンセル＋再試行（呼び出し元が再試行処理を指定している場合のみ再試行ボタンを表示）
- メッセージ本文にエラー詳細（errorDescription）と回復提案（recoverySuggestion）を表示
